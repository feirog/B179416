...
```
<!-- The following chunk can take a long time to load and may fail due to connection issues. Please do get comfortable! -->
...
antihist_sold_by_month_and_drug = antihistamines_data %>% 
  ...
  summarize(total = sum(paid_quantity)) %>% # Ensuring paid_quantity
  mutate(drug = factor(drug, levels = antihist_sold_by_drug$drug, ordered = T)) # Order the drugs as factors based on the ranked order specified in the table dataset (above)
